...
public class AdminController {
	...
	@RequestMapping("/admin/loginvue")
	public @ResponseBody Result loginVue(Manager manager, HttpSession session) {
		...
		if (adminService.validateManager(manager)) {
			...
			String sessionTime = CompositeFactory.getString(Constant.SESSION_INVALID_TIME);
			session.setMaxInactiveInterval(Integer.parseInt(sessionTime));
			return new Result(Constant.SUCCESS_CODE, Constant.LOGIN_SUCCESS);
		} else {
			return new Result(Constant.FAILED_CODE, Constant.LOGIN_FAIL);
		}
	}
	...
	public @ResponseBody Manager getcurrentUserName(HttpSession session) {
		Manager manager = (Manager) session.getAttribute(Constant.USERINFO);
		return manager;
	}
